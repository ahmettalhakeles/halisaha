...
async function loadWeeklySchedule() {
    ...
    try {
        ...
        // Pitch filtresi göster/gizle ve dinamik olarak doldur
        if (pitchFilterEl) {
            pitchFilterEl.style.display = (data.pitchCount > 1) ? 'flex' : 'none';
            if (pitchSel && data.pitchCount > 1) {
                const currentVal = pitchSel.value;
                pitchSel.innerHTML = '<option value="">Tümü</option>';
                for (let i = 1; i <= data.pitchCount; i++) {
                    pitchSel.innerHTML += `<option value="${i}">Saha ${i}</option>`;
                }
                if (currentVal && parseInt(currentVal) <= data.pitchCount) {
                    pitchSel.value = currentVal;
                }
            }
        }
...
function renderKontrolGrid(data, monday, isMobile) {
    ...
    (reservations || []).forEach(r => {
        ...
        if (/^\d{2}\.\d{2}\.\d{4}$/.test(rDate)) {
            ...
            const parsedDate = parseTurkishDateString(rDate);
            if (parsedDate) rDate = formatDateYMD(parsedDate);
        }
...
function showKontrolReservationDetail(r) {
    ...
    if(waPhone.startsWith('0')) waPhone = '9' + waPhone;
    else if(!waPhone.startsWith('90')) waPhone = '90' + waPhone;

    const modalOverlay = document.createElement('div');
    modalOverlay.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.8);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;';
    
    const modalContent = document.createElement('div');
    modalContent.style.cssText = 'background:var(--bg-card); border: 1px solid var(--border-color); border-radius:15px; width:100%; max-width:400px; padding:25px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); position:relative;';
    
    modalContent.innerHTML = `
        <button class="close-modal-btn" style="position:absolute;top:15px;right:15px;background:none;border:none;color:var(--text-muted);font-size:1.5rem;cursor:pointer;">&times;</button>
        <h3 style="color:var(--neon-green);margin-top:0;margin-bottom:20px;border-bottom:1px solid var(--border-color);padding-bottom:10px;font-size:1.3rem;">📋 Rezervasyon Detayı</h3>
        
        <div style="display:flex;flex-direction:column;gap:12px;margin-bottom:25px;font-size:0.95rem;">
            ...
            <div style="display:flex;justify-content:space-between;">
                ...
                <span style="font-weight:600;color:var(--text-light);">${phone}</span>
            </div>
            <div style="display:flex;justify-content:space-between;">
                <span style="color:var(--text-muted);">Tarih / Saat:</span>
                <span style="font-weight:600;color:var(--text-light);">${date} - ${hour}</span>
            </div>
            <div style="display:flex;justify-content:space-between;">
                <span style="color:var(--text-muted);">Saha:</span>
                <span style="font-weight:600;color:var(--text-light);">${pitch}</span>
            </div>
            <div style="display:flex;justify-content:space-between;">
                <span style="color:var(--text-muted);">Tür:</span>
                <span style="font-weight:600;color:var(--text-light);">${type}</span>
            </div>
            <div style="display:flex;justify-content:space-between;">
                <span style="color:var(--text-muted);">Ödeme:</span>
                <span style="font-weight:600;color:var(--text-light);">${payStatus}</span>
            </div>
            <div style="display:flex;justify-content:space-between;">
                <span style="color:var(--text-muted);">Fiyat:</span>
                <span style="font-weight:600;color:var(--text-light);">${price}</span>
            </div>
        </div>
        
        <a href="https://wa.me/${waPhone}" target="_blank" style="display:flex;align-items:center;justify-content:center;gap:10px;background:#25D366;color:#fff;text-decoration:none;padding:12px;border-radius:8px;font-weight:600;transition:opacity 0.2s;width:100%;box-sizing:border-box;">
