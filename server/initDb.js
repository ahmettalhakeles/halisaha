...
async function initDatabase(connection) {
    ...
    const migrations = [
        ...
        { check: "SHOW COLUMNS FROM reservations LIKE 'notification_status'", alter: "ALTER TABLE reservations ADD COLUMN notification_status VARCHAR(50) DEFAULT NULL" },
        { check: "SHOW COLUMNS FROM reservations LIKE 'customer_phone'", alter: "ALTER TABLE reservations ADD COLUMN customer_phone VARCHAR(20) DEFAULT NULL" },
        { check: "SHOW COLUMNS FROM reservations LIKE 'payment_method'", alter: "ALTER TABLE reservations ADD COLUMN payment_method VARCHAR(20) DEFAULT NULL" },
        ...
        { check: "SHOW TABLES LIKE 'email_login_tokens'", alter: "CREATE TABLE email_login_tokens (token_hash CHAR(64) NOT NULL PRIMARY KEY, user_id INT NOT NULL, expires_at DATETIME NOT NULL, consumed_at DATETIME DEFAULT NULL, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, KEY idx_email_login_user (user_id), CONSTRAINT fk_email_login_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci" },
